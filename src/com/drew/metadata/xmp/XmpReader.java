...
//    private static final String SCHEMA_DUBLIN_CORE_SPECIFIC_PROPERTIES = "http://purl.org/dc/elements/1.1/";
    ...
    {
        if (segmentBytes.length > 27 && "http://ns.adobe.com/xap/1.0/".equalsIgnoreCase(new String(segmentBytes, 0, 28)))
        {
            return true;
        }
        if (segmentBytes.length > 33 && "http://ns.adobe.com/xmp/extension/".equalsIgnoreCase(new String(segmentBytes, 0, 34)))
        {
            return true;
        }
        return false;
    ...
    public void extract(@NotNull byte[] segmentBytes, @NotNull Metadata metadata, @NotNull JpegSegmentType segmentType)
    {
        ...
        for (byte value : segmentBytes) {
            if (value == 0x00) {
                break;
            }
            signatureBuffer.append((char)value);
        }
        
        String signature = signatureBuffer.toString();
        
        if ("http://ns.adobe.com/xap/1.0/".equals(signature))
        {
            final int signatureLength = signature.length() + 1;
            if (segmentBytes.length < signatureLength + 1) {
                directory.addError(String.format("Xmp data segment must contain at least %d bytes", signatureLength + 1));
                return;                
            }
            byte[] xmlBytes = new byte[segmentBytes.length - signatureLength];
            System.arraycopy(segmentBytes, signatureLength, xmlBytes, 0, xmlBytes.length);
            extract(xmlBytes, metadata);
            
            directory._extendedXmpData = null;
            directory._extendedXmpRead = 0;
            directory._extendedXmpChecksum = null;
            try
            {
                XMPMeta xmpMeta = directory.getXMPMeta();
                directory._extendedXmpChecksum = xmpMeta.getPropertyString("http://ns.adobe.com/xmp/note/", "xmpNote:HasExtendedXMP");
            }
            catch (XMPException e)
            {
                // Nothing to do, just leave extended XMP fields null
            }
        }
        else if ("http://ns.adobe.com/xmp/extension/".equals(signature))
        {
            final int signatureLength = signature.length() + 1;
            if (segmentBytes.length < signatureLength + 32 + 4 + 4 + 1)
            {
                directory.addError(String.format("Xmp extended data segment must contain at least %d bytes", signatureLength + 32 + 4 + 4 + 1));
                return;                
            }
            
            final String checksum = new String(segmentBytes, signatureLength, 32);
            if (!checksum.equals(directory._extendedXmpChecksum))
            {
                directory.addError("Xmp extended data checksum does not match " + directory._extendedXmpChecksum + " != " + checksum);
                return;
            }
            
            final int extendedLengthIndex = signatureLength + 32;
            final int extendedOffsetIndex = extendedLengthIndex + 4;
            int extendedLength = 0;
            int extendedOffset = 0;
            for (int index = 0; index < 4; ++index)
            {
                extendedLength = (extendedLength << 8) | (segmentBytes[extendedLengthIndex + index] & 0xFF);
                extendedOffset = (extendedOffset << 8) | (segmentBytes[extendedOffsetIndex + index] & 0xFF);
            }
            
            if (directory._extendedXmpData == null)
            {
                directory._extendedXmpData = new byte[extendedLength];
            }
            else if (directory._extendedXmpData.length != extendedLength)
            {
                directory.addError("Xmp extended data length mismatch " + directory._extendedXmpData.length + " != " + extendedLength);
                directory._extendedXmpData = null;
                directory._extendedXmpRead = 0;
                directory._extendedXmpChecksum = null;
                return;
            ...
            final int preambleLength = signatureLength + 32 + 4 + 4;
            final int blockLength = segmentBytes.length - preambleLength;
            
            if (extendedOffset + blockLength > extendedLength)
            {
                directory.addError("Xmp extended data buffer overflow");
                directory._extendedXmpData = null;
                directory._extendedXmpRead = 0;
                directory._extendedXmpChecksum = null;                
            }
            
            System.arraycopy(segmentBytes, preambleLength, directory._extendedXmpData, extendedOffset, blockLength);
            directory._extendedXmpRead += blockLength;
            
            if (directory._extendedXmpRead > extendedLength)
            {
                directory.addError("Xmp extended data length exceeded " + directory._extendedXmpRead + " > " + extendedLength);
                directory._extendedXmpData = null;
                directory._extendedXmpRead = 0;
                directory._extendedXmpChecksum = null;
                return;
            ...
            if (directory._extendedXmpRead == extendedLength)
            {
                extract(directory._extendedXmpData, metadata);
                directory._extendedXmpData = null;
                directory._extendedXmpRead = 0;
                directory._extendedXmpChecksum = null;
            }
        ...
        {
            directory.addError("XMP data segment begins with unknown signature");
        }
    }
    ...
    private void processXmpTags(XmpDirectory directory, XMPMeta xmpMeta) throws XMPException
    {
        ...
        if (directory.getXMPMeta() == null)
        {
            directory.setXMPMeta(xmpMeta);
        }
        else
        {
            XMPUtils.appendProperties(xmpMeta, directory.getXMPMeta(), false, false);
        }
